...
- (void)requestFinished:(PinResponseResult *)request {
    ...
    } else if ([request.methodBack isEqualToString:@"addVotefavorite"]) {
        MyWishModel *myWishModel = [request.userInfo objectForKey:@"myWishModel"];
        myWishModel.myVoteModel.favorite_guid = 1;
        myWishModel.myVoteModel.vote_favorite += 1;
        [self.tableview reloadData];
    }  else if ([request.methodBack isEqualToString:@"removeVotefavorite"]) {
        MyWishModel *myWishModel = [request.userInfo objectForKey:@"myWishModel"];
        myWishModel.myVoteModel.favorite_guid = 0;
        myWishModel.myVoteModel.vote_favorite -= 1;
        [self.tableview reloadData];
    } else if ([request.methodBack isEqualToString:@"addPostfavorite"]) {
        MyWishModel *myWishModel = [request.userInfo objectForKey:@"myWishModel"];
        myWishModel.myPostModel.favorite_guid = 1;
        myWishModel.myPostModel.post_favorite += 1;
        [self.tableview reloadData];
    }  else if ([request.methodBack isEqualToString:@"removePostfavorite"]) {
        MyWishModel *myWishModel = [request.userInfo objectForKey:@"myWishModel"];
...
- (void)myCollectSupport:(MyWishModel *)mywishModel {
    ...
    if (mywishModel.type == PinMyCentralType_CentralComapre) {
        NSString *paramString = [NSString stringWithFormat:@"vid=%zd&uid=%zd", mywishModel.myVoteModel.vote_guid, [UserDefaultManagement instance].userId];
        if (mywishModel.myVoteModel.favorite_guid == 0) { // 添加赞
            [super pinRequestByGet:paramString withMethodName:@"addfavorite.a" withMethodBack:@"addVotefavorite" withUserInfo:userInfo withIndicatorStyle:PinIndicatorStyle_NoIndicator];
        } else { // 移除赞
            [super pinRequestByGet:paramString withMethodName:@"removefavorite.a" withMethodBack:@"removeVotefavorite" withUserInfo:userInfo withIndicatorStyle:PinIndicatorStyle_NoIndicator];
        }
    } else {
        NSString *paramString = [NSString stringWithFormat:@"pid=%zd&uid=%zd", mywishModel.myPostModel.post_guid, [UserDefaultManagement instance].userId];
        if (mywishModel.myPostModel.favorite_guid == 0) { // 添加赞
            [super pinRequestByGet:paramString withMethodName:@"addfavorite.a" withMethodBack:@"addPostfavorite" withUserInfo:userInfo withIndicatorStyle:PinIndicatorStyle_NoIndicator];
        } else { // 移除赞
            [super pinRequestByGet:paramString withMethodName:@"removefavorite.a" withMethodBack:@"removePostfavorite" withUserInfo:userInfo withIndicatorStyle:PinIndicatorStyle_NoIndicator];
        }
    }
