...
- (void)resetMyWishCell:(NSMutableArray *)collectArray {
    ...
    for (int i = 0; i < collectArray.count; i++) {
        ...
        if (myWishModel.type == PinMyCentralType_CentralComapre) { // 品选
            ...
            [myWishCellView resetMyWishVoteCellView:myWishModel.myVoteModel];
            ...
            supportImageview.image = myWishModel.myVoteModel.favorite_guid > 0 ? IMG_Name(@"supportRedSel") : IMG_Name(@"supportGray");
            supportLabel.textColor = myWishModel.myVoteModel.favorite_guid > 0 ? HEXCOLOR(pinColorPink) : HEXCOLOR(pinColorTextLightGray);
            supportLabel.text = [NSString stringWithFormat:@"%zd", myWishModel.myVoteModel.vote_favorite];
            
        } else {
            
            MyWishCellView *myWishCellView = [[MyWishCellView alloc] init];
            [myWishCellView resetMyWishPostCellView:myWishModel.myPostModel];
            [cellContentView addSubview:myWishCellView];
            ...
            if (myWishModel.type == PinMyCentralType_CentralRecommed) { // 推荐
                supportImageview.image = myWishModel.myPostModel.favorite_guid > 0 ? IMG_Name(@"supportRedSel") : IMG_Name(@"supportGray");
                supportLabel.textColor = myWishModel.myPostModel.favorite_guid > 0 ? HEXCOLOR(pinColorPink) : HEXCOLOR(pinColorTextLightGray);
            } else { // 吐槽
                supportImageview.image = myWishModel.myPostModel.favorite_guid > 0 ? IMG_Name(@"supportBlackSel") : IMG_Name(@"supportGray");

                supportLabel.textColor = myWishModel.myPostModel.favorite_guid > 0 ? HEXCOLOR(pinColorBlack) : HEXCOLOR(pinColorTextLightGray);
            }
            
            supportLabel.text = [NSString stringWithFormat:@"%zd", myWishModel.myPostModel.post_favorite];
        }
