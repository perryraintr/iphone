...
- (void)postPublishData:(NSMutableArray *)mechandiseArray {
    [self.httpService postEditPublishRequestWithGuid:self.guid imageArray:self.imageArray fileNameArray:self.fileNameArray t1:@"2" name:self.recommendTextFiled.text description:self.recommendTextView.text mechandiseArray:mechandiseArray finished:^(NSDictionary *result, NSString *message) {
    } failure:^(NSDictionary *result, NSString *message) {
    }];
}
