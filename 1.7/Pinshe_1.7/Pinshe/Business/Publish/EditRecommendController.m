...
@interface EditRecommendController () <UIImagePickerControllerDelegate, UINavigationControllerDelegate, UITextViewDelegate, UITextFieldDelegate, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UIActionSheetDelegate>
...
- (void)requestPostInfo {
    [self.httpService postInfoRequestWithGuid:self.guid finished:^(NSDictionary *result, NSString *message) {
        // 编辑页面请求网络数据
        DetailRecommendModel *detailRecommendModel = [DetailRecommendModel modelWithDictionary:result];
...
- (void)requestTagDetail {
    ...
    [self.httpService tagDetailRequestWithCurrentPage:1 finished:^(NSDictionary *result, NSString *message) {
        for (NSDictionary *dic in [result objectForKey:@"array"]) {
            ...
            for (NSNumber *number in [self.postParams objectForKey:@"myPublishTag2Array"]) {
                if ([number integerValue] == psModel.guid) {
...
- (void)postPublishData:(NSMutableArray *)mechandiseArray {
    [self.httpService postEditPublishRequestWithGuid:self.guid imageArray:self.imageArray fileNameArray:self.fileNameArray t1:@"1" name:self.recommendTextFiled.text description:self.recommendTextView.text mechandiseArray:mechandiseArray finished:^(NSDictionary *result, NSString *message) {
    } failure:^(NSDictionary *result, NSString *message) {
    }];
}
