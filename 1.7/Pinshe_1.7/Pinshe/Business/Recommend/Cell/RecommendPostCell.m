...
@implementation RecommendPostCell
...
- (void)resetRecommendPostCell:(PostModel *)model {

    self.supportButton.info = model;
    self.supportImageview.image = model.favorite_guid > 0 ? IMG_Name(@"supportRedSel") : IMG_Name(@"supportGray");
    self.supportLabel.textColor = model.favorite_guid > 0 ? HEXCOLOR(pinColorPink) : HEXCOLOR(pinColorTextLightGray);
