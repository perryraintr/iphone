...
- (void)resetRecommendCell:(DetailVoteModel *)detailVote {
    ...
    if (detailVote.product_a_guid > 0) { // 商品的品选
        [self.picImageviewLeft sd_setImageWithURL:[NSURL URLWithString:detailVote.product_a_image] placeholderImage:nil];
        [self.picImageviewRight sd_setImageWithURL:[NSURL URLWithString:detailVote.product_b_image] placeholderImage:nil];
    } else {
...
- (void)leftTap {
    if (self.detailVote.product_a_guid > 0) {
        if (self.detailBlock) {
            self.detailBlock(self.detailVote.product_a_guid);
        }
    }
}

- (void)rightTap {
    if (self.detailVote.product_b_guid > 0) {
        if (self.detailBlock) {
            self.detailBlock(self.detailVote.product_b_guid);
        }
    }
}
