...
- (void)requestFinished:(PinResponseResult *)request {
    ...
    if ([request.methodBack isEqualToString:@"product"]) {
        ...
        if (self.pinProductDetailModel.tag_t2.count == 0) {
            self.detailHeaderview.hidden = YES;
        } else {
            [self.detailHeaderview resetDetailHeaderView:self.pinProductDetailModel.tag_t2 withUserName:@"" withUserImageUrl:@"" isShit:NO];
            self.detailHeaderview.headImageview.hidden = YES;
        }
        ...
        [PINBaseRefreshSingleton instance].refreshTopProductList = 1;

    } else if ([request.methodBack isEqualToString:@"product_addfavorite"] || [request.methodBack isEqualToString:@"product_removefavorite"]) {
        [self.detailFooterview resetProductDetailWith:self.pinProductDetailModel];
        [PINBaseRefreshSingleton instance].refreshTopProductList = 1;
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section {
    if (section == 0) {
        return self.pinProductDetailModel.product_images.count > 0 ? 1 : 0;
    } else if (section == 1) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) { // 图片
        ...
        [pinProductImageCell resetPINProductImagesCell:self.pinProductDetailModel.product_images];
...
- (void)detailSupportAction {
    if (self.pinProductDetailModel.favorite_guid == 0) { // 添加赞
        self.pinProductDetailModel.favorite_guid = 1;
        self.pinProductDetailModel.product_favorite += 1;
        NSString *paramString = [NSString stringWithFormat:@"prodid=%zd&uid=%zd", self.pinProductDetailModel.product_guid, [UserDefaultManagement instance].userId];
        [super pinRequestByGet:paramString withMethodName:@"addfavorite.a" withMethodBack:@"product_addfavorite" withUserInfo:nil withIndicatorStyle:PinIndicatorStyle_NoIndicator];
        
    } else { // 移除赞
        self.pinProductDetailModel.favorite_guid = 0;
        self.pinProductDetailModel.product_favorite -= 1;
        NSString *paramString = [NSString stringWithFormat:@"prodid=%zd&uid=%zd", self.pinProductDetailModel.product_guid, [UserDefaultManagement instance].userId];
        [super pinRequestByGet:paramString withMethodName:@"removefavorite.a" withMethodBack:@"product_removefavorite" withUserInfo:nil withIndicatorStyle:PinIndicatorStyle_NoIndicator];
    }
}
...
- (void)shareDetail {
    ...
    self.pinShareView.shareImageUrl = self.pinProductDetailModel.product_image;
    self.pinShareView.shareUrl = [NSString stringWithFormat:@"%@product_detail_share.html?pid=%zd", REQUEST_HTML_URL, self.pinProductDetailModel.product_guid];
    self.pinShareView.shareContent = self.pinProductDetailModel.product_name;
