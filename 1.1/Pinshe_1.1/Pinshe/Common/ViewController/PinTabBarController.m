...
- (void)messageCountRequest {
    ...
    [super pinRequestByGet:paramsFromDictionary(paramsDic) withMethodName:@"message.a" withMethodBack:@"messageCountRequest" withUserInfo:nil withIndicatorStyle:PinIndicatorStyle_NoIndicator];
}
...
- (void)requestFinished:(PinResponseResult *)request {
    ...
    if ([request.methodBack isEqualToString:@"messageCountRequest"]) {
        int badgeNumber = [[pinBaseModel.body objectForKey:@"count"] intValue];
