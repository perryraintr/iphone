...
- (void)messageRequestWithDragup:(BOOL)isDragup withIndicatorStyle:(PinIndicatorStyle)indicatorStyle {
    NSString *paramStr = [NSString stringWithFormat:@"uid=%zd&page=%zd", [UserDefaultManagement instance].userId, isDragup ? (self.currentPage + 1) : 1];
    NSMutableDictionary *userInfo = [NSMutableDictionary dictionary];
    [userInfo setObject:[NSNumber numberWithInt:isDragup ? (self.currentPage + 1) : 1] forKey:@"currentPage"];
    [super pinRequestByGet:paramStr withMethodName:@"message.a" withMethodBack:@"message" withUserInfo:userInfo withIndicatorStyle:indicatorStyle];
}
...
- (void)requestFinished:(PinResponseResult *)request {
    ...
    PinBaseModel *pinBaseModel = [PinBaseModel modelWithJSON:requestString];
    [super requestFinished:request];
    
    if ([request.methodBack isEqualToString:@"message"]) {
...
- (NSString *)getPushDetail:(int)type {
    switch (type) {
        case 1:
            return FORWARD_PRODUCTDETAIL_VC; // 推荐详情
            break;
        case 2:
