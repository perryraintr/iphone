...
- (void)requestFinished:(PinResponseResult *)request {
    ...
    if ([request.methodBack isEqualToString:@"postDetail"]) {
        ...
        [self.detailHeaderview resetDetailHeaderView:self.detailRecommendModel.tag_t2 withUserName:self.detailRecommendModel.user_name withUserImageUrl:self.detailRecommendModel.user_avatar isShit:YES];
        ...
        self.detailFooterview.replyNumLabel.text = [NSString stringWithFormat:@"%zd", self.detailRecommendModel.post_comment + 1];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        CGFloat brandHeight = [NSString getTextHeight:(SCREEN_WITH - FITWITH(16) - 3) text:self.detailRecommendModel.post_name withLineHiehtMultipe:1.0 withLineSpacing:0 fontSize:fFont14 isSuo:NO];
        
        CGFloat descriptionHeight = [NSString getTextHeight:(SCREEN_WITH - FITWITH(27) * 2) text:self.detailRecommendModel.post_description withLineHiehtMultipe:1.0 withLineSpacing:5 fontSize:fFont14 isSuo:NO];
