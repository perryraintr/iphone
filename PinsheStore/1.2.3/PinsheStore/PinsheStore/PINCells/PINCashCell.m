...
- (void)buildingUI {
    ...
    self.amountLabel = Building_UILabelWithSuperView(self.contentView, Font(fFont18), HEXCOLOR(pinColorDarkBlack), NSTextAlignmentLeft, 0);

    self.statusLabel = Building_UILabelWithSuperView(self.contentView, Font(fFont18), HEXCOLOR(pinColorDarkBlack), NSTextAlignmentRight, 0);
...
- (void)resetCashCell:(PINCashModel *)model {
    if (model.type == -1) {
        ...
        self.amountLabel.text = [NSString stringWithFormat:@"+%.2f元", model.amount];
        self.statusLabel.textColor = HEXCOLOR(pinColorGreen);
        self.amountLabel.textColor = HEXCOLOR(pinColorGreen);
        [self layoutUI:NO];
    }
