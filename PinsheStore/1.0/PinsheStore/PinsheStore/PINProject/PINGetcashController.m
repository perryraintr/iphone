...
- (void)initBaseParams {
    self.sid = [[self.postParams objectForKey:@"sid"] intValue];
}

- (void)requestCashAdd:(NSString *)price {
    [[super findFirstResponder] resignFirstResponder];
    
    if ([PINUserDefaultManagement instance].pinUser.current < [price floatValue]) {
        [self chatShowHint:@"余额不足"];
        return;
    }
    
    [self.httpService cashAddRequestWithSid:self.sid mid:[PINUserDefaultManagement instance].pinUser.guid amount:price  finished:^(NSDictionary *result, NSString *message) {
