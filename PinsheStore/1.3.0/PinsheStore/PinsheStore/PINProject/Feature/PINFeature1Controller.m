...
- (void)initBaseParams {
    ...
    self.paramFeatureArray = [self.postParams objectForKey:@"feature1s"];
}
...
- (void)saveFeature {
    ...
    if (featureArray.count == 0) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:NSSelectorFromString(@"resetFeature1:")]) {
            SUPPRESS_PERFORMSELECTOR_LEAK_WARNING([self.delegate performSelector:NSSelectorFromString(@"resetFeature1:") withObject:featureArray]);
            [super backAction];
        }
    }
