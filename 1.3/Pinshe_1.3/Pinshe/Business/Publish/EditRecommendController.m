...
- (void)requestPostInfo {
    [super pinRequestByGet:[NSString stringWithFormat:@"pid=%zd", self.guid] withMethodName:@"post.a" withMethodBack:@"editPost" withUserInfo:nil withIndicatorStyle:PinIndicatorStyle_DefaultIndicator];
}
...
- (void)requestFinished:(PinResponseResult *)request {
    ...
    if ([request.methodBack isEqualToString:@"editPost"]) {
        // 编辑页面请求网络数据
