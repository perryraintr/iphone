...
- (void)shareDetail {
    ...
    UMSocialUrlResource *image = [[UMSocialUrlResource alloc] initWithSnsResourceType:UMSocialUrlResourceTypeImage url:self.detailVote.product_a_guid > 0 ? self.detailVote.product_a_image : self.detailVote.posta_image];
    
    NSString *shareUrl = [NSString stringWithFormat:@"%@vote_detail_share.html?vid=%zd", REQUEST_HTML_URL, self.detailVote.vote_guid];
    //分享点击链接
    ...
    [[UMSocialDataService defaultDataService]  postSNSWithTypes:@[UMShareToWechatTimeline]
                                                        content:self.detailVote.vote_name
                                                          image:nil                                                       location:nil
                                                    urlResource:image
