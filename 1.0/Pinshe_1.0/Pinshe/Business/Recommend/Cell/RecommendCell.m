...
- (void)layoutUI:(NSString *)brand description:(NSString *)description {
    ...
    CGFloat brandHeight = [NSString getTextHeight:(SCREEN_WITH - FITWITH(16) - 5) text:brand withLineHiehtMultipe:1.0 fontSize:fFont14 isSuo:NO];
...
- (void)resetRecommendCell:(DetailRecommendModel *)model {
    ...
    self.titleLabel.attributedText = resetLineHeightMultiple(1.0, model.post_name);
    self.titleImageview.hidden = YES;
    if (model.post_name.length > 0) {
        self.titleImageview.hidden = NO;
    }
    self.describeLabel.attributedText = resetLineHeightMultiple(1.0, model.post_description);
