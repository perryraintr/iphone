...
- (void)dealloc {
    ...
    [self.defaultBorderIndicator stopAnimating];
}
...
- (void)setTitle:(NSString *)title {
    ...
    float width = [super.title boundingRectWithSize:CGSizeMake(MAXFLOAT, CGRectGetHeight(self.navigationController.navigationBar.bounds))
                                         ...
                                         attributes:[NSDictionary dictionaryWithObjectsAndKeys:Font(fFont16), NSFontAttributeName, nil]
                                            context:nil].size.width;
    UILabel *titleLabel = [[UILabel alloc] initWithFrame:CGRectMake((CGRectGetWidth(self.navigationController.navigationBar.bounds) - width)/2, 0, width, CGRectGetHeight(self.navigationController.navigationBar.bounds))];
...
- (void)initBaseUI {
    ...
    self.defaultBorderIndicator = [[PINActivityIndicatorView alloc] init];
    self.defaultBorderIndicator.hidden = YES;
    [self.view addSubview:_defaultBorderIndicator];
    [self.defaultBorderIndicator mas_makeConstraints:^(MASConstraintMaker *make) {
        make.center.equalTo(self.view);
        make.width.height.equalTo(@(FITWITH(80)));
    }];
}
...
#pragma mark -
#pragma mark 网络请求
- (BaseNetworkManager *)networkManager {
    if (!_networkManager) {
        _networkManager = [[BaseNetworkManager alloc] init];
...
- (void)cancelAllRequest {
    [self.networkManager cancelAllRequest];
}
...
- (void)pinRequestByGet:(NSString *)requestParams withMethodName:(NSString *)methodName withMethodBack:(NSString *)methodBack withUserInfo:(NSMutableDictionary *)userInfo withIndicatorStyle:(PinIndicatorStyle)indicatorStyle {
    [self startActivityIndicatorView:[NSString stringWithFormat:@"%tu", indicatorStyle]];
    [self.networkManager pinRequestByGet:requestParams withMethodName:methodName withMethodBack:methodBack withUserInfo:userInfo withIndicatorStyle:indicatorStyle];
}
...
- (void)pinRequestByPost:(NSMutableDictionary *)reqDic withMethodName:(NSString *)methodName withMethodBack:(NSString *)methodBack withUserInfo:(NSMutableDictionary *)userInfo withIndicatorStyle:(PinIndicatorStyle)indicatorStyle {
    [self startActivityIndicatorView:[NSString stringWithFormat:@"%tu", indicatorStyle]];
    [self.networkManager pinRequestByPost:reqDic withMethodName:methodName withMethodBack:methodBack withUserInfo:userInfo withIndicatorStyle:indicatorStyle];
}
...
- (void)pinPostImageData:(NSMutableDictionary *)reqDic withMethodName:(NSString *)methodName withMethodBack:(NSString *)methodBack withUserInfo:(NSMutableDictionary *)userInfo withIndicatorStyle:(PinIndicatorStyle)indicatorStyle {
    [self startActivityIndicatorView:[NSString stringWithFormat:@"%tu", indicatorStyle]];
    [self.networkManager pinPostImageData:reqDic withMethodName:methodName withMethodBack:methodBack withUserInfo:userInfo withIndicatorStyle:indicatorStyle];
}

- (void)startActivityIndicatorView:(NSString *)indicatorStyle {
    if ([indicatorStyle intValue] == PinIndicatorStyle_DefaultIndicator) {
        self.defaultBorderIndicator.hidden = NO;
        [self.defaultBorderIndicator startAnimating];
    }
}
...
#pragma mark -
#pragma mark ---- AFNetworkHTTPRequestDelegate ----

- (void)requestFinished:(PinResponseResult *)request {
    [self stopActivityIndicatorView:request.indicatorStyle];
}

- (void)requestFailed:(PinResponseResult *)request {
    [self stopActivityIndicatorView:request.indicatorStyle];
    if (pinSheAppDelegate().networkType == PinNetworkType_None && [request.indicatorStyle intValue] == PinIndicatorStyle_DefaultIndicator) {
